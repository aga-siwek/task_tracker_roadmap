...
def list_status(status):
    ...
    cursor_obj = conn.cursor()
    if status == "all":
        cursor_obj.execute("SELECT description FROM TRACKER ORDER BY description;")
        output = cursor_obj.fetchall()
        for row in output:
            print("".join(row))
    else:
        cursor_obj.execute("SELECT description FROM TRACKER WHERE status=? ORDER BY description;", (status,))
        output = cursor_obj.fetchall()
        for row in output:
            print("".join(row))
